...
Before running the IsoBayes model, we structure all the required input data into a SummarizedExperiment object, Then, we preprocess it using the `input_data` function in order to create a suitable input for IsoBayes algorithm. 
...
Percolator_data_loaded = input_data(SE, path_to_tpm = tpm_path)
```
...
user_data_loaded = input_data(SE, path_to_tpm = tpm_path)
```
