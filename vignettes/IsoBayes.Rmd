...
To this aim, the data must be organized in a `.tsv` file or in a data.frame object where each row corresponds to a peptide with up to four fields:
...
To load the tsv file with user data, we just need to pass the file path or the data.frame object and set `input_type` as "other" .
```{r set path user, eval = FALSE}
Other_data_loaded <- load_data(
    path_to_peptides_psm = "/path/to/user_data.tsv",
    input_type = "other",
    path_to_tpm = tpm_path
)
```
