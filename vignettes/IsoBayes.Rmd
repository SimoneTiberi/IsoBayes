...
By default, `load_data` uses the PEP (`PEP = TRUE`); if we want to disable the PEP integration, and filter based on the FDR, we can set `PEP = FALSE` and specify an FDR cutoff, e.g., as `FDR_thd = 0.01`.

# Load the data
First, we load and process the input data using the `load_data` function.
...
Then, we have to set `abundance_type` argument to "intensities"; otherwise, the `load_data` function will ignore the AllQuantifiedPeptides.tsv file.
...
Once the file has been created, we can pass its path to `load_data` and set `input_type` as "openMS".
...
<<<<<<< HEAD
To this aim, the data must be organized in a `.tsv` file, a data.frame or a SummarizedExperiment object where each row corresponds to a peptide with up to four fields:
=======
To this aim, the data must be organized in a `.tsv` file or in a data.frame object where each row corresponds to a peptide and columns refer to:
>>>>>>> 79dadf6f8c9d93dcee6e212a52785c712f272502
