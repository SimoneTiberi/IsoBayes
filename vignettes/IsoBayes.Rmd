...
# Generate SummarizedExperiment object and preprocess input data
Before running the IsoBayes model, we structure all the required input data into a SummarizedExperiment object, Then, we preprocess it using the `load_data` function in order to create a suitable input for IsoBayes algorithm. 
...
First, we load the file and we convert it into a SummarizedExperiment object via `generate_SE` function. Then, we preprocess the input data using the `load_data` function:
```{r load psm}
SE = generate_SE(path_to_peptides_psm = path_to_peptides_psm,
                 abundance_type = "psm",
                 input_type = "metamorpheus"
                 )
PSM_data_loaded = load_data(SE, path_to_tpm = tpm_path)
```
...
Percolator_data_loaded = load_data(SE, path_to_tpm = tpm_path)
```
...
user_data_loaded = load_data(SE, path_to_tpm = tpm_path)
```
