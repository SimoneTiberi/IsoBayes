...
The data can be organized in a `.tsv` file, or a `data.frame`.
In both cases, each row corresponds to a peptide, and columns refer to:
...
* `Y`; a numeric variable indicating the peptide abundance (PSM counts or intensities).

Object `colData`, a `data.frame` object with columns:
...
Object `metadata`, a list of vectors:
