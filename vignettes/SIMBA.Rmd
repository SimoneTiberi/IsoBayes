...
## Input data
*SIMBA* works with the output of both *MetaMorpheus* (MM) [@MetaMorpheus], or *Percolator* [@Percolator] (via the *OpenMS* toolkit [@OpenMS]).
...
The code to generate the data can be found [here](https://github.com/SimoneTiberi/SIMBA/blob/main/inst/script/extdata.Rmd).

## PSM counts vs. intensities
We can run *SIMBA* on either PSM counts or intensities.
...
## PEP vs. FDR cutoff
Additionally, we suggest to provide the probability that peptides are erroneously detected (usually called PEP): *SIMBA* will use the PEP to sample the presence/absence of each peptide; this will propagate the peptide identification uncertainty throughout the inference process.
...
The AllPeptides.psmtsv file contains all the information required to run *SIMBA* with PSM counts;
...
*SIMBA* is also compatible with the PSM file from *Percolator* (from the *OpenMS* toolkit).
The [README](https://github.com/SimoneTiberi/SIMBA/tree/main#readme) shows how to use *OpenMS* tools to generate an idXML file containing PSM data.
...
Additionally, the `FDR_thd` option is not considered in \code{load_data}; FDR filtering should be performed during the data generation via *OpenMS*, as shown in the [README](https://github.com/SimoneTiberi/SIMBA/tree/main#readme).
...
* 'FDR_thd': (optional) a numeric variable indicating the FDR of each peptide;
...
By default, *SIMBA* uses one single core.
...
# Visualize results
Finally, *SIMBA* provides the `plot_relative_abundances` function to visualize differential relative abundances between transcriptomics and proteomics isoforms within a specific gene:
```{r plotting results}
