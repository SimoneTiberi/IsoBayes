input_check_plot = function(x, gene_id, plot_CI, normalize_gene){
  ...
  if(all(names(x) != c("isoform_results", "normalized_isoform_results", "gene_abundance"))){
    stop("Names of 'res_inference' should be: 'isoform_results', 'normalized_isoform_results'")
  }
  if(is.null(x$normalized_isoform_results)){
    stop("Data.frame with genes and isoform names not provided. Plotting results by gene is not possible. Before executing the 'inference' function,
