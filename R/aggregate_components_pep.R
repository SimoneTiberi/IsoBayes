    ...
    for (i in seq_len(ncores)) {
        for (j in seq_len(floor(nc / ncores))) {
            if (i < length(components)) {
                ...
                components[[i]]$EC_numeric = append(components[[i]]$EC_numeric,
                                                    a)
                components[[i]]$Y = c(components[[i]]$Y, components[[i + 1]]$Y)
                components[[i]]$PEP = c(components[[i]]$PEP,
                                        components[[i + 1]]$PEP)
                components[[i]]$M = components[[i]]$M + components[[i + 1]]$M
                components[[i]]$EC_numeric_unique = c(
                    components[[i]]$EC_numeric_unique,
                    components[[i + 1]]$EC_numeric_unique + components[[i]]$N
                )
                components[[i]]$Y_unique = c(components[[i]]$Y_unique,
                                             components[[i + 1]]$Y_unique)
                components[[i]]$PEP_unique = c(components[[i]]$PEP_unique,
                                               components[[i + 1]]$PEP_unique)
                components[[i]]$M_unique = components[[i]]$M_unique + components[[i + 1]]$M_unique
                components[[i]]$protein_length = c(components[[i]]$protein_length,
                                                   components[[i + 1]]$protein_length)
                components[[i]]$pp = c(components[[i]]$pp,
                                       components[[i + 1]]$pp)
