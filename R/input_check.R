input_check = function(path_to_peptides_psm, path_to_peptides_intensities,
  ...
  character_inputs = c(
    class(path_to_peptides_intensities), class(path_to_tpm), class(input_type),
    class(abundance_type)
  )

  if (!all(character_inputs == "character")) {
    args_name = c("path_to_peptides_psm", "path_to_peptides_intensities",
                  "path_to_tpm", "input_type", "abundance_type")
  ...
  if(!is.data.frame(path_to_peptides_psm)){
    vec_files = c(path_to_peptides_psm, path_to_peptides_intensities, path_to_tpm)
