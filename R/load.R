...
#' @param path_to_tpm (optional) a character string indicating
...
load_data = function(path_to_peptides_psm,
    ...
    if (input_type == "metamorpheus") {
        variables = c("Protein Accession", "QValue",
                      "Decoy/Contaminant/Target", "Base Sequence")
        if (PEP) {
            variables = c(variables, "PEP")
        }
        if (abundance_type == "intensities") {
            data_list = list(x = fread(path_to_peptides_intensities,
                                       data.table = FALSE), y = NULL)
            data_list = build_intensity(data_list$x, path_to_peptides_psm,
                                        variables)
        } else if (abundance_type == "psm") {
            data_list = list(x = fread(path_to_peptides_psm,
                                       select = variables, data.table = FALSE),
                             y = NULL)
    ...
    if (path_to_tpm != "") {
        protein_df_args$TPM = load_tpm(protein_df_args$protein_name,
                                       path_to_tpm)
    }
    ...
    list(PEPTIDE_DF = PEPTIDE_DF, PEPTIDE_DF_unique = PEPTIDE_DF_unique,
         PROTEIN_DF = PROTEIN_DF, PEP = PEP)
}
