...
* 'Decoy/Contaminant/Target': a character string indicating if the peptide is a decoy, contaminant or, or target;
* 'PSM Count (unambiguous, <0.01 q-value)': a numeric variable indicating how many times the peptide has been identified (peptide abundance);
...
Below is the script used to create `AllPeptides.psmtsv` and `AllQuantifiedPeptides.tsv`, both available inside SIMBA package.
[Codice MM]
...
# Code to get jurkat_isoform_kallisto.tsv
[code kallisto?]
...
df = as.data.frame(data.table::fread(data_path))
tpm = as.data.frame(data.table::fread(tmp_path))
...
data.table::fwrite(df, "jurkat_isoform_kallisto.tsv")
```

# Code to create map_iso_gene.csv
